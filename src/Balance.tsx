...
const DraggableIcon = (props: {
  ...
  return (
    <Box component="div">
      ...
      >
        {icon.icon}3 star 2 star 1 star Super buff
      </Box>
    </Box>
  );
};
  ...
  (Object.keys(clone) as (keyof typeof clone)[]).forEach(
    (key: BalanceKey) =>
      (clone[key] = clone[key].filter(
        (v) =>
          v.kind === k && v.icon.toLowerCase().includes(search.toLowerCase())
      ))
  );
  return clone;
}
...
export const NavBar = (props: {
  ...
  const { setTab, currentTab, canSubmit, showSubmit, submit, onSearch } = props;
